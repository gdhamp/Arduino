...
// i2cLCD constructor is called).

i2cLCD::i2cLCD(uint8_t rs, uint8_t rw, uint8_t enable,
...
i2cLCD::i2cLCD(uint8_t rs, uint8_t enable,
			     uint8_t d0, uint8_t d1, uint8_t d2, uint8_t d3,
			     uint8_t d4, uint8_t d5, uint8_t d6, uint8_t d7)
{
...
i2cLCD::i2cLCD(uint8_t data, uint8_t clock, uint8_t latch ) {
  _i2cAddr = 255;
...
void i2cLCD::init(uint8_t fourbitmode, uint8_t rs, uint8_t rw, uint8_t enable,
			 uint8_t d0, uint8_t d1, uint8_t d2, uint8_t d3,
			 uint8_t d4, uint8_t d5, uint8_t d6, uint8_t d7)
{
  _rs_pin = rs;
...
#else
inline void i2cLCD::write(uint8_t value) {
  send(value, HIGH);
}
#endif
...
void  i2cLCD::_digitalWrite(uint8_t p, uint8_t d) {
  if (_i2cAddr != 255) {
    // an i2c command
...
void i2cLCD::setBacklight(uint8_t status) {
  // check if i2c or SPI
...
void  i2cLCD::_pinMode(uint8_t p, uint8_t d) {
  if (_i2cAddr != 255) {
    // an i2c command
    _i2c.pinMode(p, d);
  } else if (_SPIclock != 255) {
...
void i2cLCD::write8bits(uint8_t value) {
  for (int i = 0; i < 8; i++) {
    _pinMode(_data_pins[i], OUTPUT);
    _digitalWrite(_data_pins[i], (value >> i) & 0x01);
  }
